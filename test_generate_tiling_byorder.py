...
tiling_method = 'random-target-set'
tiling_set_size = 100
...
test_tiling, tiling_completeness, remaining_targets = tl.generate_tiling_byorder(
	...
	tiling_order=tiling_order, tiling_set_size=tiling_set_size,
	ra_min=330., ra_max=53., dec_min=-36., dec_max=-26.,
...
print 'TILING STATS'
print '------------'
print 'Tiling complete in %d:%02.1f' % (int(floor(time_to_complete/60.)),
	time_to_complete % 60.)
...
ax2.set_title('%d targets, %s tiling, %s unpicking (%s)' % (len(all_targets),
...
fig.savefig('test_tiling_%s_%s%s.pdf' % (tiling_method, alloc_method, supp_str),
	fmt='pdf')
