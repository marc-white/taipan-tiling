...
def execute(cursor, date_start, date_end, output_loc='.', prep_db=True,
    ...
    while curr_date <= date_end:
        ...
        if curr_date not in weather_fails.keys():
            ...
            weather_fail_thresh = np.percentile(weather_fails.values(),
                                                weather_loss * 100.)
        if curr_date == sim_start + initial_fibre_end:
            tp._alter_fibres(no_fibres='upgrade')
            # Lose 14 days to the upgrade
...
if __name__ == '__main__':
    ...
    sim_start = datetime.date(2020, 7, 1)
    sim_end = datetime.date(2022, 1, 18)
    global_start = datetime.datetime.now()
    prior_lowz_end = datetime.date(2021, 7, 1) - sim_start
    initial_fibres_end = datetime.date(2021, 1, 29) - datetime.timedelta(days=15.) - sim_start
    ...
    execute(cursor, sim_start, sim_end,
            ...
            initial_fibre_end=initial_fibres_end,
