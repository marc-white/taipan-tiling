...
FIBRES_NORMAL.sort()
FIBRES_NORMAL_INITIAL = FIBRES_NORMAL[:]
""":obj:`list` of :obj:`int`: List of normal (i.e. non-guide) fibres installed
...
if len(FIBRES_NORMAL) + len(FIBRES_GUIDE) != INSTALLED_FIBRES:
    raise Exception('The number of fibre positions defined '
                    '(%d guides + %d normal = %d) does'
                    ' not match the set value for INSTALLED_FIBRES (%d). '
                    'Please '
                    'check the fibre configuration variables in core.py.' %
                    (len(FIBRES_GUIDE), len(FIBRES_NORMAL),
                     len(FIBRES_GUIDE) + len(FIBRES_NORMAL),
                     INSTALLED_FIBRES, ))
...
def _alter_fibres(no_fibres='initial'):
    """
    ...
    .. warning: This function is designed to increase the number of fibres to
                150, or decrease it back down to 107.

    Parameters
    ----------
    no_fibres : :obj:`str`, either ``'initial'`` or ``'upgrade'``
    ...
    ------
    :any:`ValueError`
        Raised if ``no_fibres`` isn't ``"initial"`` or ``"upgrade"``.
    """
    # Input checking
    if no_fibres not in ['initial', 'upgrade']:
        raise ValueError('no_fibres must be one of "initial", "upgrade"')
    ...
    if no_fibres == 'initial':
        if len(FIBRES_NORMAL) == len(FIBRES_NORMAL_INITIAL):
            return  # No action required

        FIBRES_NORMAL = FIBRES_NORMAL_INITIAL[:]
